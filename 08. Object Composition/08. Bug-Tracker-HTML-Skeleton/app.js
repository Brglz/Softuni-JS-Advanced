function solve() {
    ...
    function report(author, description, reproducible, severity) {
        let status = 'Open'
        const statusSpan = elementCreate('span', `${status} | ${severity}`, { className: 'status' })
        const element = elementCreate('div', [
            elementCreate('div', elementCreate('p', description), { className: 'body' }),
            elementCreate('div', [
                elementCreate('span', `Submited by: ${author}`, { className: 'author' }),
                statusSpan
            ], { className: 'title' })
        ], {
